...
defineOptions({ name: 'FlowFolderDialog' });
import api from '@/api';
...
const values = reactive({
  name: (props.flow as any)?.name ?? null,
  icon: (props.flow as any)?.icon ?? 'folder',
  description: (props.flow as any)?.description ?? null,
  color: (props.flow as any)?.color ?? null,
});
...
watch(
  ...
  (newValue, oldValue) => {
    if (isEqual(newValue, oldValue) === false) {
      values.name = (props.flow as any)?.name ?? null;
      values.icon = (props.flow as any)?.icon ?? 'folder';
      values.description = (props.flow as any)?.description ?? null;
      values.color = (props.flow as any)?.color ?? null;
    }
  },
);
...
<template>
  ...
  >
    ...
    <v-card>
      <v-card-title v-if="!flow">{{ t('create_folder') }}</v-card-title>
...
.flow-key { --v-input-font-family: var(--theme--fonts--monospace--font-family); }
</style>
