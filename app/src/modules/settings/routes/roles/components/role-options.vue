...
const props = defineProps<{
  role: Role & { hidden?: boolean; collapse?: 'open' | 'collapsed' | 'locked' | null };
  hasNestedRoles: boolean;
}>();
  ...
  <v-menu placement="left-start" show-arrow>
    ...
    <v-list>
      <v-list-item
        ...
        clickable
        v-tooltip="hasNestedRoles ? t('cannot_delete_non_empty_folder') : undefined"
        @click="!hasNestedRoles ? (confirmDelete = true) : null"
      >
        <v-list-item-icon>
  ...
  <v-dialog v-model="confirmDelete" @esc="confirmDelete = false">
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-button kind="danger" @click="deleteRole">
          {{ t('delete_label') }}
        </v-button>
      </v-card-actions>
    </v-card>
